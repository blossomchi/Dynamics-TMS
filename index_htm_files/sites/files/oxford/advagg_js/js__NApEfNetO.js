    ...
    attach: function(context, settings) {
      ...
      function addHamburger() {
        if (!$(".hamburger-wrapper").exists()) {
          ...
          $("#block-funnelback-search-form").after('<div class="hamburger-wrapper"><a href="#" class="hamburger-btn">Menu</a></div>');
      ...
      function toggleSearchSize() {
        // Hide the text input
        $('#block-funnelback-search-form .form-item-funnelback-search-field .form-text').hide();

        // Add a class
        $('#block-funnelback-search-form', context).addClass('toggle');
        $('#block-funnelback-search-form .form-submit').click(function(e){
          // If the button icon is clicked and the text input is missing add it.
          if($('#block-funnelback-search-form', context).hasClass('toggle')) {
            e.preventDefault();
            $('#block-funnelback-search-form .form-item-funnelback-search-field .form-text').fadeIn(100).focus();
            $('#block-funnelback-search-form', context).removeClass('toggle');
          }
          // If the search input is clicked and we're in responsive mode and the search is empty don't search
          if ($('#block-funnelback-search-form .form-item-funnelback-search-field .form-text').val() == '' && $('body').hasClass('responsive-mode') ) {
            e.preventDefault();
          }
        });
        // If you focusout of the search item hide the search again.
        $('#block-funnelback-search-form .form-item-funnelback-search-field .form-text').focusout(function() {
          if ($('body', context).hasClass('sticky-header-collapsed') || $('body', context).hasClass('responsive-mode')) {
            $(this).fadeOut(300, function(){
              $('#block-funnelback-search-form').addClass('toggle');
            });
          }
        });
      }
        ...
        $(window).scroll(throttle(function() {
          ...
          if ($(document).scrollTop() >= topMenuTriggerFixed && isEligibleForSticky) {
            ...
            if (!$('body').hasClass('sticky-header-collapsed') && $('body').hasClass('desktop')) {
              $('#block-funnelback-search-form .form-item-search-keys .form-text').show();
              $('#block-funnelback-search-form').removeClass('toggle');
            }
